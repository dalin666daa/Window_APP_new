...
from login import LoginWindow  # Import the LoginWindow
from reports import ReportsPage  # Import the ReportsPage class
from report_selection_dialog import ReportSelectionDialog  # Import the custom dialog
# from master import MasterPage  # Import the MasterPage class
...
class HomePage(QWidget):
    def __init__(self):
        ...
        # self.master_page = MasterPage()  # Create an instance of MasterPage
        
        self.init_home_page()

        self.stacked_widget.addWidget(self.home_page)
        self.stacked_widget.addWidget(self.reports_page)
        self.stacked_widget.addWidget(self.master_page)  # Add master page
    ...
    def init_home_page(self):
        ...
        layout.addLayout(second_card_layout)
    ...
    def on_login(self):
        ...
        if login_window.exec_() == QDialog.Accepted:  # Check if login was successful
            self.stacked_widget.setCurrentWidget(self.master_page)  # Switch to master page

    def on_select_report(self):
        dialog = ReportSelectionDialog(reports_folder='reports/', parent=self)
        dialog.report_selected.connect(self.on_report_selected)  # Connect the signal
        dialog.exec_()

    def on_report_selected(self, file_path):
        self.reports_page.load_reports(file_path)  # Load the selected Excel file
